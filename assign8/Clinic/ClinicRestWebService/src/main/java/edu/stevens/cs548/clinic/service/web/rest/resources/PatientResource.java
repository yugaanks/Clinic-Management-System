...
public class PatientResource {
	...
	public TreatmentRepresentation getPatientTreatment(@PathParam("id") String id, @PathParam("tid") String tid) {
		try {
			TreatmentDto tdto = patientService.getTreatment(Long.parseLong(id), Long.parseLong(tid));
			TreatmentRepresentation trep = new TreatmentRepresentation(tdto, context);
			return trep;
		} catch (PatientServiceExn e) {
			throw new WebApplicationException("Not Found", 404);
		}
